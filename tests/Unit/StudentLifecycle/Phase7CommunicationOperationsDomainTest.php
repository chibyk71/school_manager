...
it('incomplete enrollment reminders are idempotent and skip finalized', function () {
    ...
    // Logs use audience-scoped reminder_key ({base}:parent) plus reminder_base_key.
    $baseKey = 'enrollment_incomplete:'.$idle->id;
    $successLogs = \App\Models\NotificationLog::query()
        ->where('school_id', $school->id)
        ->where('success', true)
        ->where(function ($q) use ($baseKey) {
            $q->where('metadata->reminder_key', $baseKey)
                ->orWhere('metadata->reminder_key', $baseKey.':parent')
                ->orWhere('metadata->reminder_key', $baseKey.':admin')
                ->orWhere('metadata->reminder_base_key', $baseKey);
        })
        ->count();
