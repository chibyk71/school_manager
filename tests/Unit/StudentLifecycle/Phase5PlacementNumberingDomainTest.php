...
function buildPhase5Schema(): void
{
    ...
    Schema::create('student_session_placements', function (Blueprint $t) {
        ...
        $t->index(['class_section_id', 'is_current', 'left_at']); $t->index(['student_id', 'academic_session_id', 'is_current']);
    });
    ...
    Schema::create('student_class_section_pivot', function (Blueprint $t) {
        $t->id(); $t->uuid('student_id'); $t->uuid('class_section_id'); $t->uuid('academic_session_id')->nullable(); $t->boolean('is_current')->default(false); $t->date('enrolled_at')->nullable(); $t->date('left_at')->nullable(); $t->timestamps();
    });
}
...
it('allows the same formatted admission number in different schools', function () {
    $a = p5School('A', 'AAA'); $b = p5School('B', 'BBB');
    IdGenerator::generate('admission_number', $a); IdGenerator::generate('admission_number', $b);
...
it('rejects mutating an assigned admission number', function () {
    $school = p5School(); $student = p5Student($school);
    $student->admission_number = 'ADM/2026/00001'; $student->save();
    $student->admission_number = 'HACKED'; $student->save();
})->throws(ValidationException::class);

it('enforces database uniqueness of admission numbers within a school', function () {
    $school = p5School(); $s1 = p5Student($school); $s2 = p5Student($school);
    $s1->admission_number = 'ADM/2026/00001'; $s1->save();
    $s2->admission_number = 'ADM/2026/00001'; $s2->save();
})->throws(\Illuminate\Database\QueryException::class);

it('does not regenerate admission number on subsequent ensure calls', function () {
    $school = p5School(); $student = p5Student($school); ['alloc' => $alloc] = p5Services();
...
it('requires id_sequences for admission_number and does not fall back to cache', function () {
    Schema::dropIfExists('id_sequences'); $school = p5School();
    IdGenerator::generate('admission_number', $school);
})->throws(\RuntimeException::class);

it('auto-allocates first available section by sort_order', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $full = p5Section($school, $level, 'A', 1, 10); $next = p5Section($school, $level, 'B', 30, 20);
...
it('rejects automatic allocation when all sections are full', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    p5Section($school, $level, 'A', 1, 10); ['alloc' => $alloc] = p5Services();
    $filler = p5Student($school);
...
it('treats capacity 0 as unlimited', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $section = p5Section($school, $level, 'A', 0, 10); ['alloc' => $alloc] = p5Services();
    for ($i = 0; $i < 5; $i++) {
        $student = p5Student($school);
...
it('rejects full section without override', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $section = p5Section($school, $level, 'A', 1, 10); ['alloc' => $alloc] = p5Services();
    $filler = p5Student($school);
...
it('rejects section from wrong class level', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $ss = p5SchoolSection($school);
    $levelA = p5Level($school, $ss, 'JSS1'); $levelB = p5Level($school, $ss, 'JSS2');
    $sectionB = p5Section($school, $levelB, 'A', 30, 10); $student = p5Student($school);
    ['alloc' => $alloc] = p5Services();
...
it('rejects cross-school section placement', function () {
    $schoolA = p5School('A', 'AAA'); $schoolB = p5School('B', 'BBB'); $user = p5User();
    $session = p5Session($schoolA); $levelA = p5Level($schoolA);
    $sectionB = p5Section($schoolB, p5Level($schoolB), 'A', 30, 10); $student = p5Student($schoolA);
    ['alloc' => $alloc] = p5Services();
...
it('preserves placement history on manual move within the same session', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $secA = p5Section($school, $level, 'A', 30, 10); $secB = p5Section($school, $level, 'B', 30, 20);
    $student = p5Student($school); $enrollment = p5Enrollment($school, $session, $student);
    ['alloc' => $alloc] = p5Services();
...
it('does not close current placement from a different academic session', function () {
    $school = p5School(); $user = p5User();
    $session1 = p5Session($school, '2025/2026'); $session2 = p5Session($school, '2026/2027');
    $level = p5Level($school); $secA = p5Section($school, $level, 'A', 30, 10); $secB = p5Section($school, $level, 'B', 30, 20);
    $student = p5Student($school); ['alloc' => $alloc] = p5Services();
...
it('assigns registration numbers and preserves history on regenerate', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $section = p5Section($school, $level, 'A', 30, 10); $student = p5Student($school);
...
it('does not produce duplicate sequence values under sequential allocation', function () {
    $school = p5School(); $numbers = [];
    for ($i = 0; $i < 20; $i++) { $numbers[] = IdGenerator::generate('admission_number', $school); }
...
it('allows only one of two concurrent processes to claim the final capacity slot', function () {
    ...
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $section = p5Section($school, $level, 'A', 1, 10);
    $s1 = p5Student($school); $s2 = p5Student($school);
    $e1 = p5Enrollment($school, $session, $s1); $e2 = p5Enrollment($school, $session, $s2);
    ...
    $codes = [];
    foreach ($pids as $pid) { pcntl_waitpid($pid, $status); $codes[] = pcntl_wexitstatus($status); }
...
it('prevents duplicate active registration assignments under sequential race-style claims', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $section = p5Section($school, $level, 'A', 30, 10); ['reg' => $reg] = p5Services();
    $s1 = p5Student($school); $s2 = p5Student($school);
    $scopeKey = $reg->buildScopeKey($school, $session->id, $level->id, $section->id);
    IdSequence::query()->create(['type' => 'registration_number', 'school_id' => $school->id, 'scope_key' => $scopeKey, 'year' => 2026, 'last_value' => 0]);
...
it('rolls back student admission number and placement when outer finalization transaction aborts', function () {
    $school = p5School(); $user = p5User(); $session = p5Session($school); $level = p5Level($school);
    $section = p5Section($school, $level, 'A', 30, 10); $student = p5Student($school);
    $enrollment = p5Enrollment($school, $session, $student); ['alloc' => $alloc] = p5Services();
    $thrown = false;
    try {
    ...
    } catch (\RuntimeException $e) { $thrown = $e->getMessage() === 'force outer rollback'; }
    ...
    for ($i = 0; $i < 2; $i++) {
        ...
        if ($pid === 0) {
            ...
            try {
                ...
                file_put_contents($tmp . '.out' . $i, $number); exit(0);
            } catch (Throwable $e) {
                file_put_contents($tmp . '.out' . $i, 'ERR:' . $e->getMessage()); exit(1);
            }
        }
        $pids[] = $pid;
    }
    foreach ($pids as $pid) { pcntl_waitpid($pid, $status); }
    $a = @file_get_contents($tmp . '.out0'); $b = @file_get_contents($tmp . '.out1');
...
it('rejects ensureAdmissionNumber when student belongs to a different school', function () {
    $schoolA = p5School('A', 'AAA'); $schoolB = p5School('B', 'BBB');
    $student = p5Student($schoolA); ['alloc' => $alloc] = p5Services();
...
it('rejects assign when student belongs to a different school', function () {
    $schoolA = p5School('A', 'AAA'); $schoolB = p5School('B', 'BBB'); $user = p5User();
    $session = p5Session($schoolB); $level = p5Level($schoolB); $section = p5Section($schoolB, $level, 'A', 30, 10);
    $student = p5Student($schoolA); ['reg' => $reg] = p5Services();
    expect(fn () => $reg->assign($student, $schoolB, [
        'academic_session_id' => $session->id, 'class_level_id' => $level->id, 'class_section_id' => $section->id,
