 ...
 * Prerequisites are minimal stubs; the addresses table itself comes only from the migration.
 */
...
it('creates the SQLite partial unique primary index from the migration', function () {
    ...
    $indexes = collect(DB::select("PRAGMA index_list('addresses')"))->pluck('name')->all();
...
it('defaults is_primary to false from the migration', function () {
    ...
    expect((bool) DB::table('addresses')->where('id', $id)->first()->is_primary)->toBeFalse();
});
