<template>
    <DataTable :value="terms" :rows="10">
        <template #header>
            <div class="flex items-center flex-col-reverse">
                <h4>Terms/Semester for {{ data.session }} Academic Session</h4>
                <Button icon="pi pi-plus" label="Add Semester" size="small" class="ml-auto" @click="modals.open('term')" />
            </div>
        </template>
        <Column field="id" header="ID" />
        <Column field="name" header="Name" />
        <Column field="start_date" header="Start Date" />
        <Column field="end_date" header="End Date" />
        <Column header="Action">
            <template #body="slotProps">
                <div class="flex items-center gap-x-2">
                    <Button icon="pi pi-pencil" size="small" severity="secondary" />
                    <Button icon="pi pi-trash" size="small" severity="danger" />
                </div>
            </template>
        </Column>
        <template #empty>
            <div class="p-4 text-center">
                <p>No terms/semesters found</p>
            </div>
        </template>
    </DataTable>
    <ModalWrapper :form id="term" resource="term" header="Create Term/Semester" modal>
        <div class="">
            <InputWrapper required label="Name" v-model="form.name" :error="form.errors.name" name="name" field_type="text" />
            <InputWrapper required label="Start Date" v-model="form.start_date" :error="form.errors.start_date" name="start_date" field_type="date" />
            <InputWrapper required label="End Date" v-model="form.end_date" :error="form.errors.end_date" name="end_date" field_type="date" />
            <InputWrapper field_type="select" name="status" required label="Status" :error="form.errors.status">
                <template #input>
                    <Select option-label="label" option-value="value" fluid v-model="form.status" :options="[{label: 'Pending', value: 'pending'}, {label: 'Active', value: 'active'}, {label: 'Completed', value: 'completed'}]" />
                </template>
            </InputWrapper>
        </div>
    </ModalWrapper>
</template>

<script setup lang="ts">
import axios from 'axios';
import { Button, Column, DataTable, Select } from 'primevue';
import { inject, onMounted, ref } from 'vue';
import ModalWrapper from '../ModalWrapper.vue';
import { useForm } from '@inertiajs/vue3';
import { modals } from '@/helpers';
import InputWrapper from '@/Components/inputs/InputWrapper.vue';
const data = ref<{ session: string }>({ session: '' });

const dialogRef = inject<{ value: { data: { session: string, session_id: number } } }>('dialogRef');

const terms = ref<{ id: number; name: string; start_date: string; end_date: string }[]>([]);

onMounted(() => {
    axios.get(route('term.index',dialogRef!.value.data.session_id))
        .then(response => response.data)
        .then(data => {
            terms.value = data;
        })
        .catch(error => {
            console.error('Error fetching terms:', error);
        });
    data.value = dialogRef!.value.data;
})

const form = useForm({
    name: '',
    start_date: null,
    end_date: null,
    status: 'pending',
    academic_session_id: dialogRef!.value.data.session_id
})
</script>
