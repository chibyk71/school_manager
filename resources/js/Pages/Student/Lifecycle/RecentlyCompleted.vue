...
function changeWindow(days: number) {
    router.get(route('lifecycle.recently-completed'), { within_days: days }, {
        preserveState: true,
        replace: true,
    });
}
...
<template>
    <Head title="Recently Completed" />

    <div class="space-y-6 p-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Recently Completed</h1>
                <p class="mt-1 text-sm text-gray-500">{{ total }} activity item(s) in the last {{ within_days }} days</p>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="item in items" :key="item.type + '-' + item.id">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600">{{ item.at || '—' }}</td>
                    ...
                    <tr v-if="!items.length">
                        <td colspan="4" class="px-4 py-8 text-center text-sm text-gray-500">No recent activity.</td>
                    </tr>
                </tbody>
            </table>
        </div>
