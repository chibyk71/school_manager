...
<template>
    ...
    <AuthenticatedLayout title="Custom Fields" :crumb="[{ label: 'Settings' }, { label: 'System' }, { label: 'Custom Fields' }]"
        ...
        <AdvancedDataTable :endpoint="route('settings.system.custom-fields', { resource: selectedResource })"
            ...
            :initial-params="{ resource: selectedResource }" @row-reorder="handleReorder" data-key="id" :export-filename="`custom-fields-${selectedResource}`" :actions="actionsButtons" :bulk-actions="bulkActions">
            <!-- Custom empty state -->
            <template #empty>
