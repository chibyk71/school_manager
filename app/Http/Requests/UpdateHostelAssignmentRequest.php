...
/**
 * Request class for updating an existing hostel assignment.
 *
 * Validates input data for updating a hostel assignment, ensuring it is scoped to the active school,
 * checks room capacity, and prevents duplicate active assignments for the same student.
 */
class UpdateHostelAssignmentRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        $assignment = $this->route('hostelAssignment');
        return [
            'hostel_room_id' => [
                'required',
                'exists:hostel_rooms,id',
                function ($attribute, $value, $fail) use ($school, $assignment) {
                    ...
                    // Check room capacity, excluding current assignment
            ...
            'student_id' => [
                ...
                Rule::unique('hostel_assignments')->where(function ($query) use ($assignment) {
    ...
    {
        return [
            'student_id.unique' => 'This student is already assigned to another active hostel room.',
        ];
    }
}
