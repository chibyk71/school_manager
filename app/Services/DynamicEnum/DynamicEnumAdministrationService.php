...
class DynamicEnumAdministrationService
{
    ...
    public function detail(string $key, ?School $school, bool $canManage, bool $canManageGlobals): array
    {
        ...
        $options = $resolved->options->map(function (ResolvedDynamicEnumOption $opt) use (
            ...
            // Ownership-specific presentation so the UI can seed edit forms from
            // the row that will actually be mutated (not the effective merge).
            return array_merge($opt->toArray(), [
                'tenant_label' => $tenantRow?->label,
                'tenant_sort_order' => $tenantRow?->sort_order,
                'tenant_color' => $tenantRow?->color,
                'tenant_icon' => $tenantRow?->icon,
                'school_label' => $schoolRow?->label,
                'school_sort_order' => $schoolRow?->sort_order,
                'school_color' => $schoolRow?->color,
                'school_icon' => $schoolRow?->icon,
