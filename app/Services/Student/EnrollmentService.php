 ...
 * - Profile resolution is conservative (exact match only, no fuzzy/cross-tenant).
...
class EnrollmentService
{
    ...
    public function updateBiodata(Enrollment $enrollment, User $actor, array $biodata): Enrollment
    {
        return DB::transaction(function () use ($enrollment, $actor, $biodata) {
            ...
            $meta = $locked->meta ?? [];
            $meta['biodata'] = array_merge($meta['biodata'] ?? [], $this->sanitizeBiodata($biodata));
    ...
    {
        $blockers = [];
        $details = ['biodata' => null, 'requirements' => [], 'finance' => null, 'duplicate_active' => null];
    ...
    public function finalize(Enrollment $enrollment, User $actor): Enrollment
    {
        return DB::transaction(function () use ($enrollment, $actor) {
            ...
            $profile = $this->resolveOrCreateProfile($biodata, $actor);
            ...
            if ($existingStudent) {
                ...
                if ($this->studentHasColumn('first_name')) {
                    $student->setAttribute('first_name', $biodata['first_name'] ?? null);
                    $student->setAttribute('last_name', $biodata['last_name'] ?? null);
                }
            ...
            return $locked->fresh(['student', 'admission', 'requirementInstances.definition', 'academicSession']);
        });
    }
    ...
    protected function resolveOrCreateProfile(array $biodata, User $actor): Profile
    {
        $email = isset($biodata['email']) ? strtolower(trim((string) $biodata['email'])) : null;

        if ($email) {
            $existing = Profile::query()->whereRaw('LOWER(email) = ?', [$email])->first();
            if ($existing) {
                return $existing;
            }
        }
        ...
        $profile = new Profile();
        $profile->first_name = $biodata['first_name'] ?? '';
        $profile->last_name = $biodata['last_name'] ?? '';
        if ($email) {
            $profile->email = $email;
        }
        if (! empty($biodata['phone'])) {
            $profile->phone = $biodata['phone'];
        }
        if (! empty($biodata['date_of_birth'])) {
            $profile->date_of_birth = $biodata['date_of_birth'];
        }
        if (! empty($biodata['gender'])) {
            $profile->gender = $biodata['gender'];
        }
        $profile->save();

        activity()->performedOn($profile)->causedBy($actor)->log('profile.created_via_enrollment_finalize');
    ...
    protected function sanitizeBiodata(array $biodata): array
    {
        $allowed = [
            ...
            'date_of_birth', 'gender', 'nationality', 'address_line_1',
    ...
    {
        try {
            return \Illuminate\Support\Facades\Schema::hasColumn('students', $column);
        } catch (\Throwable) {
            return false;
        }
    }
