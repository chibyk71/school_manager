...
class PlacementAllocationService
{
    ...
    public function changeSection(
        ...
        return DB::transaction(function () use ($student, $school, $destinationSectionId, $actor, $options) {
            ...
            $sessionHint = $options['academic_session_id'] ?? null;
            $currentQuery = StudentSessionPlacement::query()
                ->where('student_id', $student->id)
                ->where('is_current', true)
                ->whereNull('left_at');
            if ($sessionHint) {
                $currentQuery->where('academic_session_id', $sessionHint);
            }
            ...
            if ((string) $sessionId !== (string) $current->academic_session_id) {
                throw ValidationException::withMessages([
    ...
    protected function assertCapacityAvailable(ClassSection $section, string $sessionId, User $actor, bool $override): void
    {
        ...
        throw ValidationException::withMessages([
            'class_section_id' => 'Selected section is at capacity.',
        ]);
    }
    ...
    {
        try {
            if (method_exists($actor, 'isAbleTo') && $actor->isAbleTo('placements.capacity_override')) {
                return true;
            }
            if (method_exists($actor, 'hasPermission') && $actor->hasPermission('placements.capacity_override')) {
                return true;
            }
        } catch (\Throwable $e) {
    ...
    protected function createPlacement(
        ...
        if ($closePrevious) {
            // Current is per student + academic session — never close another session's current row.
    ...
    {
        $level = ClassLevel::query()->withoutGlobalScopes()->find($classLevelId);
        if (!$level) {
            throw ValidationException::withMessages([
    ...
    protected function assertStudentIsActiveForPlacementChange(Student $student): void
    {
        ...
        if (in_array($status, ['withdrawn', 'transferred', 'graduated', 'completed'], true)) {
            throw ValidationException::withMessages([
                'student' => 'Student status does not allow placement changes.',
            ]);
        }
    }
}
