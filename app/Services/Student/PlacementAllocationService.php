...
class PlacementAllocationService
{
    ...
    public function allocateForEnrollment(
        ...
        return DB::transaction(function () use ($enrollment, $student, $school, $actor, $options) {
            ...
            if ($requestedSectionId) {
                ...
                $this->assertCapacityOrOverride($section, $override, $actor, $sessionId);
            } else {
                $section = $this->selectSectionWithCapacity($classLevelId, $school, $sessionId);
                $override = false;
            }
    ...
    /**
     * Active occupancy for a section within a single academic session.
     * Placements from other sessions must not consume capacity.
     */
    public function activeOccupancy(ClassSection $section, string $academicSessionId, bool $forUpdate = false): int
    {
    ...
    public function selectSectionWithCapacity(string $classLevelId, School $school, string $academicSessionId): ClassSection
    {
        ...
        foreach ($sections as $section) {
            if ($this->hasAvailableCapacity($section, $academicSessionId, forUpdate: true)) {
                return $section;
            }
        }
    ...
    public function hasAvailableCapacity(ClassSection $section, string $academicSessionId, bool $forUpdate = false): bool
    {
