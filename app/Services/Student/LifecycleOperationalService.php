...
class LifecycleOperationalService
{
    ...
    public function dashboardCounts(School $school, ?AcademicSession $session = null): array
    {
        ...
        $sectionsNearCapacity = ClassSection::query()
            ...
            ->where('capacity', '>', 0)
            ->whereRaw(
                "(capacity - (SELECT COUNT(*) FROM student_session_placements ssp WHERE ssp.class_section_id = class_sections.id AND ssp.is_current = 1 AND ssp.academic_session_id = ?)) <= (CASE WHEN 2 > CAST(capacity * 0.15 AS INTEGER) THEN 2 ELSE CAST(capacity * 0.15 AS INTEGER) END)",
                [$sessionId]
            )
            ->count();
     ...
     */
    /**
     * @param  list<string>|null  $categories  Subset of: applications, admissions, enrollments.
     *                                        Null means all categories (caller is fully authorized).
     */
    public function needsAttention(School $school, int $limit = 50, ?array $categories = null): array
    {
        $schoolId = $school->id;
        $items = collect();
        ...
        $includeApps = in_array('applications', $categories, true);
        $includeAdm = in_array('admissions', $categories, true);
        $includeEnr = in_array('enrollments', $categories, true);
        ...
        if ($includeAdm) {
            $totalMatching += (clone $offersBase)->count() + (clone $acceptedBase)->count();
        }
        if ($includeEnr) {
            $totalMatching += (clone $enrBase)->count();
        }
    ...
    /**
     * @param  list<string>|null  $categories  Subset of: admissions, enrollments.
     * @return array{items: list<array>, total: int}
     */
    public function upcomingDeadlines(School $school, int $withinDays = 14, int $limit = 50, ?array $categories = null): array
    {
        $schoolId = $school->id;
     ...
     * @return array{items: list<array>, total: int}
     */
    public function recentlyCompleted(School $school, int $withinDays = 14, int $limit = 50, ?array $categories = null): array
    {
        $schoolId = $school->id;
    ...
    public function applicationsQuery(School $school, array $filters = [])
    {
        ...
        // review_state maps to status buckets when domain uses review statuses
        if (! empty($filters['review_state'])) {
            $review = $filters['review_state'];
            if ($review === 'awaiting' || $review === 'pending') {
                $q->whereIn('status', [
                    StudentApplication::STATUS_SUBMITTED,
                    StudentApplication::STATUS_UNDER_REVIEW,
                    StudentApplication::STATUS_PENDING,
                ...
                $q->where('status', StudentApplication::STATUS_APPROVED);
            } elseif ($review === 'rejected') {
                $q->where('status', StudentApplication::STATUS_REJECTED);
            } else {
    ...
    public function admissionsQuery(School $school, array $filters = [])
    {
        ...
        if (! empty($filters['acceptance_state'])) {
            $state = $filters['acceptance_state'];
            if ($state === 'awaiting') {
                $q->whereIn('status', [Admission::STATUS_OFFERED, Admission::STATUS_PENDING]);
            } else {
                $q->where('status', $state);
            }
        }
        if (! empty($filters['deadline_from'])) {
            $q->where('acceptance_deadline', '>=', $filters['deadline_from']);
        }
        if (! empty($filters['deadline_to'])) {
            $q->where('acceptance_deadline', '<=', $filters['deadline_to']);
        }
    ...
    {
        // Reuse authoritative filtered query (same filters as exports).
    ...
    {
        // Same authoritative filters as admissionsQuery / AdmissionsExport.
    ...
    {
        // Same authoritative filters as enrollmentsQuery / EnrollmentsExport.
    ...
    {
        // Prefer authoritative placement.school_id (Phase 6).
    ...
    public function placementReport(School $school, array $filters = []): array
    {
        ...
        if ($sessionId) {
            $near->whereRaw(
                "(capacity - {$capacitySql}) <= (CASE WHEN 2 > CAST(capacity * 0.15 AS INTEGER) THEN 2 ELSE CAST(capacity * 0.15 AS INTEGER) END)",
                [$sessionId]
            );
        } else {
            $near->whereRaw(
        ...
        // Full section utilization via single aggregated join (no silent truncation, no N+1).
        $placedSub = StudentSessionPlacement::query()
            ->selectRaw('class_section_id, COUNT(*) as placed_count')
            ->where('is_current', true)
            ->when($sessionId, fn ($q) => $q->where('academic_session_id', $sessionId))
        ...
        $utilization = ClassSection::query()
            ...
            ->where('capacity', '>', 0)
            ->leftJoinSub($placedSub, 'placed', function ($join) {
                $join->on('class_sections.id', '=', 'placed.class_section_id');
            })
            ->orderBy('class_sections.name')
            ->get([
                'class_sections.id',
                'class_sections.name',
                'class_sections.class_level_id',
                'class_sections.capacity',
                'placed.placed_count',
            ])
            ->map(function ($section) {
                $capacity = (int) $section->capacity;
                $placed = (int) ($section->placed_count ?? 0);
                ...
                return [
                    ...
                    'utilization' => $capacity > 0 ? round($placed / $capacity, 4) : null,
                ];
            })
            ->values()
            ->all();
