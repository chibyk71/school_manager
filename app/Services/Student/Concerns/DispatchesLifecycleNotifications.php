...
trait DispatchesLifecycleNotifications
{
    ...
    protected function recipientAddress(object $recipient, ?string $channel = null): string
    {
        ...
        // Plain email for mail-channel logs (FILTER_VALIDATE_EMAIL / human-readable).
        // applyRecipientFilter still matches bare email and mail: prefix forms.
        $email = $this->recipientEmail($recipient);
        if ($email !== '') {
    ...
    protected function logDispatch(
        ...
        try {
            ...
            // Always persist a non-null notifiable pair. AnonymousNotifiable has no model key;
            // schemas (and focused fixtures) often disallow null morph columns, and a silent
            // insert failure would leave successful dispatches unlogged — breaking reminder
            // idempotency on the next scheduler pass.
            if ($recipient instanceof Model && $recipient->getKey()) {
                $notifiableType = $recipient->getMorphClass();
                $notifiableId = $recipient->getKey();
            } else {
                $notifiableType = AnonymousNotifiable::class;
                $hash = sha1($this->recipientIdentity($recipient).'|'.($channel ?? ''));
                $notifiableId = sprintf(
                    '%s-%s-%s-%s-%s',
                    substr($hash, 0, 8),
                    substr($hash, 8, 4),
                    '5'.substr($hash, 13, 3),
            ...
            NotificationLog::query()->create([
                ...
                'channel' => $channel,
                'provider' => $channel === 'mail' ? 'mail' : ($meta['provider'] ?? null),
                ...
                'error' => $error,
                'segments' => 1,
                'metadata' => array_merge($meta, (static function () use ($meta, $context) {
                    $extra = [
                        'lifecycle_type' => $context->getMorphClass(),
                        'lifecycle_id' => (string) $context->getKey(),
                    ];
                    $key = $meta['reminder_key'] ?? null;
                    if (is_string($key) && $key !== '') {
                        if (str_ends_with($key, ':parent') || str_ends_with($key, ':admin')) {
                            $extra['reminder_base_key'] = preg_replace('/:(parent|admin)$/', '', $key);
                        } else {
                            $extra['reminder_base_key'] = $key;
                        }
                    }
